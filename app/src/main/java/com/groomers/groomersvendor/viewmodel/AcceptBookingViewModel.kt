...
class AcceptBookingViewModel(application: Application) : AndroidViewModel(application) {
    private val _modelAccept = MutableLiveData<ModelAccept>()
    val modelAccept: LiveData<ModelAccept> = _modelAccept
    ...
    fun acceptBooking(apiService: ApiService,accessToken: String,booking_id: String,slug: String) {
        _isLoading.postValue(true)
        viewModelScope.launch {
            try {
                val response = apiService.acceptBooking("Bearer $accessToken",booking_id,slug)
